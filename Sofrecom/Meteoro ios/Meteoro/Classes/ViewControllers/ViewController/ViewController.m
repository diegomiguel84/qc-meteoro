...
@synthesize scrollView,thePageControl,viewControllers;
...
- (void)InicializarPaginador {
    ...
    if(kNumberOfPages > 0)
    {
        ...
        else [self loadScrollViewWithPage:page];
...
-(void)ScheduledMethod
{
    ...
    for(int i=0; i< [Config getInstance].cities.count; i++)
    {
        ...
        NSLog( [[Config getInstance] dateToString:aCity.lastDateUpdate]);
...
{
    CityForecastView *controller = [viewControllers objectAtIndex:sender.orden];
    if ((NSNull *)controller == [NSNull null]) {
        controller = [[CityForecastView alloc] initWithCity:sender];
        [viewControllers replaceObjectAtIndex:sender.orden withObject:controller];
        //[controller release];
